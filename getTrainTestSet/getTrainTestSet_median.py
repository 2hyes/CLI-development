...
def getXdataframe(X):
    ...
    for i in range(len(X)):
        ...
        if ( isLastWeekOfThisMonth(X, i) ):
            ...
            # 달의 마지막 주차이면, 해당 달의 데이터들의 중앙값을 저장. 
            keyword1 = np.median(keyword1_stack)
            keyword2 = np.median(keyword2_stack)
            keyword3 = np.median(keyword3_stack)
            keyword4 = np.median(keyword4_stack)
            keyword5 = np.median(keyword5_stack)
            google = np.median(google_stack)
            naver = np.median(naver_stack)
...
if __name__ == "__main__":
    ...
    pickle.dump(X_train, open('./dataset/dataset_median/X_train.pkl','wb'))
    pickle.dump(X_test, open('./dataset/dataset_median/X_test.pkl','wb'))
    pickle.dump(y_train, open('./dataset/dataset_median/y_train.pkl','wb'))
    pickle.dump(y_test, open('./dataset/dataset_median/y_test.pkl','wb'))
    pickle.dump(ccsi, open('./dataset/dataset_median/ccsi.pkl','wb'))
    pickle.dump(predictors, open('./dataset/dataset_median/predictors.pkl','wb'))
